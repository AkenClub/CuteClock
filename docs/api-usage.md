...
- **敏感接口保护**：房间灯光控制和时钟亮度调节接口需要 API 密钥验证
- **请求头验证**：使用 `X-API-Key` 请求头传递密钥
- **灵活配置**：支持用户自定义 API 密钥，与配套小程序/Web 项目对应

### 安全建议

- ✅ **内网部署**：建议在内网环境中使用，避免暴露到公网
- ✅ **密钥同步**：如需修改默认密钥，请同步更新配套的小程序和 Web 项目
- ✅ **访问控制**：限制可信设备访问
...
| `/room-light` | 🔐 需要密钥 | 控制房间灯光 |
| `/clock`      | 🔐 需要密钥 | 调节时钟亮度 |
| `/pc-power`   | 🔐 需要密钥 | 控制电脑电源 |
...
- 默认密钥与配套的小程序、Web 项目对应，如需修改请同步更新
- 如自定义密钥，建议使用字母数字组合，长度 8 位以上
- 不要将密钥泄露给他人

### 使用方法

对于需要认证的接口，在 HTTP 请求头中加入：
...
2. **请求头格式**: 确保使用 `X-API-Key` 而不是其他名称
3. **密钥字符**: 检查是否有特殊字符或空格
